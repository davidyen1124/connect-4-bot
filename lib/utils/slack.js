...
const im = Promise.promisifyAll(slack.im)
...
module.exports = {
  ...
  sendLetsPlayMessage: async (token, playerId, opponentId) => {
    const result = await im.openAsync({
      token,
      user: playerId
    })
    const channel = result.channel.id
    const message = `:space_invader: Let's play \`Connect 4\` with <@${opponentId}>.\n:mailbox_with_mail: Checkout red badge on the sidebar.`
    await sendMessage(token, channel, message)
  },
  sendMessage
}
